...
/*var customer_name=$("#customer_name_id").val();
									...
									var site_address = $("#site_address_id").val();
									var site_units = $("#unit_new_id").val();
									
									
									
									 
									if(customer_name==""&&site_address==""&&site_units==""){
										var errstr="Please fill atleast one search input";
										$("#err_msg_changebilling").html(errstr);
										return false;
									}else{
										$("#err_msg_changebilling").html("");
									}
									...
									*/
									...
									//var billingSearchKey=$("#billingSearchKey_id").val().trim();
									//var billingSearchValue=$("#billingSearchValue_id").val().trim();
									...
									/*if(billingSearchKey=="NAME"){
										customer_name=billingSearchValue;
										site_address="";
									} else if(billingSearchKey=="ADDRESS"){
										site_address=billingSearchValue;
										customer_name="";
									}
									url : "/searchaddress?site_address="+site_address+"&site_units="+site_units+"&customer_name="+customer_name+"&TIMESTAMP="+timestamp,
									*/
									$("#tbody").html("");
									var timestamp=$("#timestamp_id").val();
									...
									$.ajax({
										...
										for(var key in data.msg){
											...
											str = str+'<tr><td><b>'+obj.customerNumber+'</b><br/>'+obj.name+'</td><td>'+obj.address+'<br/>Unit:<b>'+obj.addressUnit+'</b></td><td><input type="button" class="add_site_class btn btn-primary btn-flat" id='+key+' value="Select" /></td></tr>';
											i++;
										}
										
										if(i>0){
											var strhead="<thead><tr class='bg-gray-light'><th class='padding-none-r-10'>Customer</th><th class='padding-none-r-10'>Address</th><th>&nbsp;</th></tr></thead>";
